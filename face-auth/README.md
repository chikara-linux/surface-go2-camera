...
### 起動ロジックの改良（第 0〜2 段、2026-09-16 導入済み）

再検討の最終案を実装し、[review-2026-09-16.md](review-2026-09-16.md) の机上レビューを経て
同日 17:56 に導入した（plasma-mobile `+chikara3`）。直後の実測: 画面点灯から発光まで
1.7 秒（従来 2.6 秒）、打ち切り → 失敗 → 再開は 1 ms 以内、成功直後の空 PIN 送信は
終了に倒された。部品は 4 つ。
